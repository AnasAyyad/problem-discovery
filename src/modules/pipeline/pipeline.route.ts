...
import {
  ...
  requestPipelineJobStop,
  startPipelineJob,
} from "./pipeline-job.service.js";
...
export const registerPipelineRoutes: FastifyPluginAsync = async (app) => {
  app.post("/pipeline/run", async (request) => {
    ...
    return runPipeline({
      ...
      ...(Array.isArray(body?.keywords) && body.keywords.length > 0
        ? { discoveryKeywords: body.keywords }
        : {}),
      ...(Array.isArray(body?.webUrls) && body.webUrls.length > 0
        ? { webUrls: body.webUrls }
        : {}),
      ...(typeof body?.ingestLimit === "number"
        ? { ingestLimit: body.ingestLimit }
        : {}),
      ...(typeof body?.embedLimit === "number"
        ? { embedLimit: body.embedLimit }
        : {}),
      ...(typeof body?.extractLimit === "number"
        ? { extractLimit: body.extractLimit }
        : {}),
    });
  });
  ...
  app.post("/pipeline/jobs/:jobId/stop", async (request) => {
    const params = request.params as { jobId: string };
    return requestPipelineJobStop(Number(params.jobId));
  });
};
